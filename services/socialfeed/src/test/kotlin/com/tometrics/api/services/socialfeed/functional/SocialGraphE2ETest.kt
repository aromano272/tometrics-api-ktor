...
class SocialFeedE2ETest : BaseE2ETest() {
    ...
    fun testGetConnectionsForSpecificUser() = runApp {
        ...
        postAndAssert<Unit, Unit>(
            url = "/api/v1/socialfeed/follow/$user2Id",
            accessToken = user1Tokens.access,
            request = null
        )

        postAndAssert<Unit, Unit>(
            url = "/api/v1/socialfeed/follow/$user3Id",
            accessToken = user1Tokens.access,
            request = null
        )

        postAndAssert<Unit, Unit>(
            url = "/api/v1/socialfeed/follow/$user1Id",
            accessToken = user2Tokens.access,
            request = null
        )

        val user1Connections = getAndAssert<SocialConnections>(
            url = "/api/v1/socialfeed/connections",
            accessToken = user1Tokens.access
        )

        val user2Connections = getAndAssert<SocialConnections>(
            url = "/api/v1/socialfeed/connections/$user2Id",
            accessToken = user1Tokens.access
        )

        val user3Connections = getAndAssert<SocialConnections>(
            url = "/api/v1/socialfeed/connections/$user3Id",
            accessToken = user1Tokens.access
        )
    ...
    fun testStatusEndpoint() = runApp {
        val response = testClient.get("/api/v1/socialfeed/status")
